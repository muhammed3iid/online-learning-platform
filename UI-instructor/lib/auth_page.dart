...
import 'package:ui_instructor/instructor_page.dart';
...
class _AuthPageState extends State<AuthPage> {
  ...
  Future<void> _signIn() async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost:8081/api/instructor/login'),
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => InstructorPage(userData: responseData)),
      );
    } else {
  ...
  Future<void> _signUp() async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost:8081/api/instructor/signup'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(data),
        ...
        child: Center(
          child: Column(
            ...
            children: [
              const Center(
                  child: Text(
                'Instructor Authentication Page',
